...
struct media_transport {
	...
	uint8_t			sp_gain;	/* Transport speaker gain (headset only) */
	uint8_t			mic_gain;	/* Transport mic gain (headset only) */
	gboolean		read_lock;
	gboolean		write_lock;
	gboolean		in_use;
						...
						DBusMessageIter *value)
{
	if (g_strcmp0(property, "NREC") == 0) {
		...
		emit_property_changed(transport->conn, transport->path,
					MEDIA_TRANSPORT_INTERFACE,
					"MicrophoneGain", DBUS_TYPE_BOOLEAN,
					&transport->mic_gain);
		return 0;
	}
